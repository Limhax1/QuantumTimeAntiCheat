...
@CheckInfo(name = "AimAssist (O)", description = "Checks for consistent aiming patterns and sudden snaps.")
public class AimAssistO extends Check {
    ...
    public AimAssistO(PlayerData data) {
        super(data);
    }
                ...
                if (!exempt) {
                    if ((averageYawChange > ANGLE_THRESHOLD && yawConsistency > CONSISTENCY_THRESHOLD) ||
                        ...
                        if (buffer > BUFFER_LIMIT) {
                            fail(String.format("Suspicious aiming. AvgYaw=%.2f, AvgPitch=%.2f, YawCons=%.2f, PitchCons=%.2f, DeltaYaw=%.2f, DeltaPitch=%.2f",
