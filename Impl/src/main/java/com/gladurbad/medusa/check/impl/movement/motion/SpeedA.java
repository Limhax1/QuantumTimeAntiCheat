...
public final class SpeedA extends Check {
    ...
    public void handle(final Packet packet) {
        if(packet.isPosition() || packet.isPosLook()) {
            ...
            debug("Speed " + speed);
            if(speed > 0.65 && !data.getPlayer().hasPotionEffect(PotionEffectType.SPEED) && !isExempt(ExemptType.FLYING, ExemptType.VELOCITY, ExemptType.TELEPORT, ExemptType.SLIME)) {
